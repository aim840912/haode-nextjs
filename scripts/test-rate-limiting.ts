...
class RateLimitTester {
  ...
  async testBasicRateLimit(): Promise<boolean> {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      console.log(`  請求 ${i + 1}: ${result.allowed ? '允許' : '拒絕'} (剩餘: ${result.remaining})`);
    }
  ...
  async testIdentifierStrategies(): Promise<boolean> {
    ...
    for (const { strategy, expected } of configs) {
      ...
      console.log(`  策略 ${expected}: 請求1識別符=${result1.identifier}, 請求2識別符=${result2.identifier}`);
  ...
  async testConfigLoading(): Promise<boolean> {
    ...
    for (const path of testPaths) {
      ...
      if (config) {
        configsLoaded++;
        console.log(`  ${path}: 限制=${config.maxRequests}/${config.windowMs}ms, 策略=${config.strategy}`);
      } else {
  ...
  async testMonitoring(): Promise<boolean> {
    // 記錄一些測試事件
    await rateLimitMonitor.recordRateLimitEvent(
      '127.0.0.1',
      '/api/test',
      'ip',
      false,
