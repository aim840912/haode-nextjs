...
async function cleanupAuditLogs() {
  try {
    ...
    // 顯示最早和最新的記錄時間
    const dates = viewListLogs
      .map(log => new Date(log.created_at))
      .sort((a, b) => a.getTime() - b.getTime())
    console.log(
      `   時間範圍: ${dates[0].toLocaleString('zh-TW')} ~ ${dates[dates.length - 1].toLocaleString('zh-TW')}\n`
    )
    ...
    // 4. 顯示優化後的統計
    const { data: allLogsAfter } = await supabase.from('audit_logs').select('action')
