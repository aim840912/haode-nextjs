...
class AuditFixVerifier {
    ...
    try {
      ...
      for (const resourceType of testTypes) {
        try {
          ...
          this.log(
            `   ❌ ${resourceType}: ${error instanceof Error ? error.message : error}`,
            'error'
          )
        }
      }
      ...
      await supabaseAdmin.from('audit_logs').delete().eq('user_name', 'Verification Test')
  ...
  async testOriginalTypes(): Promise<void> {
    try {
      ...
      // 清理測試記錄
      await supabaseAdmin.from('audit_logs').delete().eq('user_name', 'Original Type Test')
    } catch (error) {
      this.log(
        `❌ 測試原有類型時發生錯誤: ${error instanceof Error ? error.message : error}`,
        'error'
      )
    }
  }
