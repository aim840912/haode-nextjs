  ...
  try {
    ...
    if (supabaseProducts.length > 0) {
      ...
      supabaseProducts.forEach((product, index) => {
        console.log(
          `     ${index + 1}. ${product.name} (isActive: ${product.isActive}, showInCatalog: ${product.showInCatalog})`
        )
      })
    }
    ...
    if (filteredProducts.length !== rawProducts.length) {
      console.log('   被過濾掉的產品:')
      const hiddenProducts = rawProducts.filter(
        (p: any) => !p.isActive || p.showInCatalog === false
      )
