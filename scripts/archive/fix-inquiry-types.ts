...
async function fixInquiryTypes() {
  try {
    ...
    // 2. 查找包含產品項目的詢價單
    const { data: inquiriesWithItems } = await supabase.from('inquiry_items').select('inquiry_id')
    ...
    if (uniqueInquiryIds.length > 0) {
      ...
      if (error) {
        ...
        if (updateResult && updateResult.length > 0) {
          console.log(
            '📋 更新的詢價單:',
            updateResult.map(i => `${i.customer_name}(${i.id.slice(0, 8)})`)
          )
        }
      }
    }
    ...
    // 6. 最終統計
    const { data: finalInquiries } = await supabase.from('inquiries').select('inquiry_type')
