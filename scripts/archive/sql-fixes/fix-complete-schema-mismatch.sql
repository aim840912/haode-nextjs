...
-- 完整修復資料庫架構不匹配問題
-- ========================================
-- 目的：移除所有不存在欄位的引用，使函數與實際資料庫 schema 完全一致
-- 執行方式：在 Supabase Dashboard > SQL Editor 中執行
-- ========================================
--
-- 根據實際 products 表結構，以下欄位存在：
-- ✅ id, name, description, category, price, price_unit, unit_quantity
-- ✅ stock, is_active, image_url, images (jsonb)
-- ✅ show_in_catalog, created_at, updated_at
--
-- 以下欄位不存在，需要從 RPC 函數中移除：
-- ❌ sku (已在前一個腳本修復)
-- ❌ is_on_sale
-- ❌ original_price
-- ❌ sale_end_date
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '========================================';
  RAISE NOTICE '✅ 函數已完全修復：移除所有不存在欄位';
  RAISE NOTICE '========================================';
  RAISE NOTICE '';
  RAISE NOTICE '變更內容：';
  RAISE NOTICE '  ✓ INSERT 語句只包含實際存在的欄位';
  RAISE NOTICE '  ✓ 移除: sku, is_on_sale, original_price, sale_end_date';
  RAISE NOTICE '  ✓ SELECT 結果只返回存在的欄位';
  RAISE NOTICE '  ✓ 保持完整的錯誤處理機制';
  RAISE NOTICE '';
