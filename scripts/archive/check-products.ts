...
class ProductDiagnostics {
  ...
  async analyzeFilterResults(allProducts: any[]): Promise<void> {
    ...
    this.log(
      `📈 is_active = true 的產品: ${activeProducts.length} 個`,
      activeProducts.length > 0 ? 'success' : 'warning'
    )

    // 模擬前端的額外過濾
    const catalogProducts = activeProducts.filter(p => p.show_in_catalog !== false)
    this.log(
      `📈 同時滿足 is_active = true 且 show_in_catalog 不為 false 的產品: ${catalogProducts.length} 個`,
    ...
    if (catalogProducts.length === 0) {
      ...
      const hiddenProducts = allProducts.filter(p => !p.is_active || p.show_in_catalog === false)
  ...
  async testApiEndpoints(): Promise<void> {
    ...
    try {
      ...
      const normalResponse = await fetch(
        `${this.supabaseUrl.replace(/\.supabase\.co.*/, '.vercel.app')}/api/products`
      )
      ...
      const adminResponse = await fetch(
        `${this.supabaseUrl.replace(/\.supabase\.co.*/, '.vercel.app')}/api/products?admin=true`
      )
      if (adminResponse.ok) {
