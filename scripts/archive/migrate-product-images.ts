...
async function moveImagesFromTempToProduct(
  ...
  try {
    ...
    for (const file of tempFiles) {
      ...
      try {
        // 複製檔案到新位置
        const { error: moveError } = await supabase.storage.from('products').move(oldPath, newPath)
        ...
        if (moveError) {
          ...
          const { data: publicUrlData } = supabase.storage.from('products').getPublicUrl(newPath)
...
async function findTempFoldersForProduct(
  productId: string,
  ...
  try {
    ...
    // 列出所有 temp- 資料夾
    const { data: allFolders, error } = await supabase.storage.from('products').list('')
...
async function migrateProductImages(
  ...
  try {
    ...
    for (const product of products) {
      ...
      // 檢查產品是否已經有正確位置的圖片
      const { data: existingFiles } = await supabase.storage.from('products').list(product.id)
