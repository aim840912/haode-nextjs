...
async function addColumns() {
  try {
    ...
    if (columnError) {
      ...
      console.log("ALTER TABLE inquiries ADD COLUMN IF NOT EXISTS inquiry_type VARCHAR(20) DEFAULT 'product' CHECK (inquiry_type IN ('product', 'farm_tour'));");
      console.log('');
      ...
      console.log('CREATE INDEX IF NOT EXISTS idx_inquiries_inquiry_type ON inquiries(inquiry_type);');
