...
const ENVIRONMENT_CONFIG: EnvironmentConfig = {
  enableInDevelopment:
    process.env.NODE_ENV !== 'development' || process.env.ENABLE_DEV_RATE_LIMITING === 'true',
  productionWhitelist: [
    // 內部服務 IP
    '127.0.0.1',
    '::1',
  ...
  developmentWhitelist: ['127.0.0.1', '::1', 'localhost'],
}
...
export const SPECIAL_PATHS = {
  /** 需要特殊處理的路徑 */
  WEBHOOK_PATHS: ['/api/payment/callback/**', '/api/webhooks/**'],
  ...
  /** 只在生產環境啟用限制的路徑 */
  PRODUCTION_ONLY: ['/api/debug/**', '/api/test/**'],
}
...
export function getRateLimitConfig(pathname: string): RateLimitConfig | null {
  // 檢查是否為排除路徑
  if (
    SPECIAL_PATHS.EXCLUDED_PATHS.some(
      path =>
        pathname.startsWith(path.replace('/**', '')) || pathname.match(path.replace('/**', '.*'))
    )
  ) {
    return null
  }
  ...
  if (
    process.env.NODE_ENV !== 'production' &&
    SPECIAL_PATHS.PRODUCTION_ONLY.some(path => pathname.startsWith(path.replace('/**', '')))
  ) {
    return null
  }
...
// 開發環境下輸出配置資訊
if (process.env.NODE_ENV === 'development' && process.env.DEBUG_RATE_LIMITING === 'true') {
