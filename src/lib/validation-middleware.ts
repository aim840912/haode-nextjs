...
export interface ValidationConfig<TBody = unknown, TQuery = unknown, TParams = unknown>
  extends Omit<BaseValidationConfig<TBody, TQuery, TParams>, 'body' | 'query' | 'params'> {
  /** Body 驗證 schema */
...
export type ValidationResult<
  ...
  TQuery = unknown,
  TParams = unknown,
> = BaseValidationResult<TBody, TQuery, TParams>
...
) => Promise<NextResponse | Response | TResponse>
...
async function validateRequestBody<T>(
  ...
  try {
    ...
    if (!result.success) {
      ...
      throw new ValidationError(`${options.errorPrefix || '請求資料'}驗證失敗: ${result.error}`)
    }
    ...
    // JSON 解析錯誤
    throw new ValidationError(`${options.errorPrefix || '請求資料'}格式錯誤: 無效的 JSON 格式`)
  }
}
...
function validateQueryParams<T>(
  ...
  if (!result.success) {
    ...
    throw new ValidationError(`查詢參數驗證失敗: ${result.error}`)
  }
...
function validateRouteParams<T>(
  ...
  schema: z.ZodSchema<T>,
  options: ValidationConfig<unknown, unknown, T>
): T {
  ...
  if (!result.success) {
    ...
    throw new ValidationError(`路由參數驗證失敗: ${result.error}`)
  }
...
export function withValidation<
  ...
  TParams = unknown,
>(
  handler: ValidatedApiHandler<TResponse, TBody, TQuery, TParams>,
  config: ValidationConfig<TBody, TQuery, TParams>
): ValidatedApiHandler<TResponse, TBody, TQuery, TParams> {
  return async (request: NextRequest, context?: { params?: TParams }) => {
    try {
      const validated: ValidationResult<TBody, TQuery, TParams> = {}
...
class ValidationMetrics {
  ...
  getMetrics(): Record<
    string,
    { success: number; failure: number; total: number; successRate: number }
  > {
    const result: Record<
      string,
      { success: number; failure: number; total: number; successRate: number }
