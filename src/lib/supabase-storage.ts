...
export async function initializeStorageBucket() {
  ...
  try {
    ...
    const bucketExists = buckets?.some(
      (bucket: SupabaseStorageBucket) => bucket.name === STORAGE_BUCKET
    )
    ...
    dbLogger.error('初始化 storage bucket 失敗', error as Error, {
      metadata: { bucketName: STORAGE_BUCKET },
    })
    throw error
  }
}
...
export async function deleteProductImages(productId: string): Promise<ProductImageDeletionResult> {
  try {
    ...
    if (!productId.startsWith('temp-')) {
      ...
      try {
        const { data: allFolders } = await supabase.storage.from(STORAGE_BUCKET).list('')

        if (allFolders) {
          const tempFolders = allFolders
            .filter((folder: SupabaseStorageFile) => folder.name.startsWith('temp-'))
            .map((folder: SupabaseStorageFile) => folder.name)

          possibleFolders.push(...tempFolders)
        }
      } catch (listAllError) {
        dbLogger.warn('無法列出所有資料夾', {
          metadata: { productId, error: (listAllError as Error).message },
        })
      }
    }
    ...
    for (const folder of possibleFolders) {
      try {
        ...
        // 建立要刪除的檔案路徑列表
        const filePaths = files.map((file: SupabaseStorageFile) => `${folder}/${file.name}`)

        // 批量刪除所有圖片
        const { error: deleteError } = await supabase.storage.from(STORAGE_BUCKET).remove(filePaths)

        if (!deleteError) {
          allDeletedFiles.push(...files.map((f: SupabaseStorageFile) => f.name))
          totalDeletedCount += filePaths.length
          dbLogger.info('成功刪除資料夾圖片', {
            metadata: { folder, deletedCount: filePaths.length, productId },
          })
        ...
        dbLogger.warn('處理資料夾時發生錯誤', {
          metadata: { folder, productId, error: (folderError as Error).message },
        })
      }
    }
    ...
    const errorMessage =
      error instanceof SupabaseStorageError ? error.message : '刪除產品圖片過程發生未知錯誤'
