...
export class AdvancedRateLimiter {
  ...
  async checkRateLimit(
    ...
    try {
      // 首先嘗試使用主要存儲（KV）
      let currentCount = await this.store.get(key);
      ...
      // 回退到記憶體存儲
      try {
        let currentCount = await this.fallbackStore.get(key);
        let count = currentCount ? parseInt(currentCount) : 0;
   ...
   * 為 API 路由創建包裝器
   */
  wrapHandler<T extends (...args: any[]) => Promise<Response | NextResponse>>(
    handler: T,
    config: RateLimitConfig
  ): T {
