...
export class CacheManager {
  ...
  static async set<T>(key: string, data: T, options: CacheOptions = {}): Promise<void> {
    ...
    try {
      ...
      cacheLogger.warn('KV cache write error', {
        metadata: { error: (error as Error).message, key, ttl },
      })
    }
  ...
  static async delete(key: string): Promise<void> {
    try {
      ...
      cacheLogger.warn('KV cache delete error', {
        metadata: { error: (error as Error).message, key },
      })
    }
  ...
  static async deletePattern(pattern: string): Promise<void> {
    try {
      ...
      cacheLogger.warn('KV cache pattern delete error', {
        metadata: { error: (error as Error).message, pattern },
      })
    }
