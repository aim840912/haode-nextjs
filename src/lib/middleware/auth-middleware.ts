...
export function validateOrigin(request: NextRequest): boolean {
  ...
  // 如果都沒有匹配，記錄詳細資訊以協助調試
  authLogger.warn('CSRF Origin validation failed', {
    metadata: {
      origin: origin || 'null',
  ...
  if (process.env.VERCEL && !origin && referer) {
    try {
      const refererUrl = new URL(referer)
      // 檢查是否來自相同的 Vercel 域名
      if (refererUrl.hostname.includes('vercel.app') || refererUrl.hostname === host) {
...
export function requireAuth(handler: (req: AuthenticatedRequest) => Promise<Response>) {
  return async (request: NextRequest) => {
    ...
    if (!user) {
      return new Response(JSON.stringify({ error: '未授權訪問，請先登入' }), {
        status: 401,
...
export function requireAdmin(handler: (req: AuthenticatedRequest) => Promise<Response>) {
  return async (request: NextRequest) => {
    ...
    if (!user.isAdmin) {
      return new Response(JSON.stringify({ error: '需要管理員權限才能執行此操作' }), {
        status: 403,
        headers: { 'Content-Type': 'application/json' },
      })
    }
...
export class CSRFTokenManager {
  ...
  static validateToken(request: NextRequest): {
    ...
    // 檢查是否都存在
    if (!headerToken) {
      authLogger.warn('CSRF Missing header token', {
        metadata: {
    ...
    if (!cookieToken) {
      authLogger.warn('CSRF Missing cookie token', {
        metadata: {
          path: request.nextUrl.pathname,
          method: request.method,
    ...
    if (headerToken !== cookieToken) {
      authLogger.warn('CSRF Token mismatch', {
        metadata: {
          path: request.nextUrl.pathname,
