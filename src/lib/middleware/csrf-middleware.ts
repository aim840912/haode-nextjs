...
export async function validateCSRFAdvanced(request: NextRequest): Promise<CSRFValidationResult> {
  try {
        ...
        response: NextResponse.json(
          {
            ...
            details: process.env.NODE_ENV === 'development' ? tokenValidation.reason : undefined,
          },
          { status: 403 }
        ),
      }
    ...
    if (!userAgent || userAgent.length < 10) {
      authLogger.warn('CSRF Suspicious request with invalid User-Agent', {
        metadata: {
          userAgent,
    ...
    const validation = advanced
      ...
      : (() => {
          ...
          return {
            ...
            response: tokenValidation.isValid
              ? undefined
              : NextResponse.json({ error: 'CSRF 驗證失敗', success: false }, { status: 403 }),
          }
        })()
