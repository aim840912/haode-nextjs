...
export function useScheduleCalendar(): UseScheduleCalendarReturn {
  ...
  const updateFilteredEvents = useCallback((eventsData: ScheduleCalendarEvent[], filter: string) => {
    ...
    logger.debug(`更新過濾後的事件: 過濾器=${filter}, 總數=${eventsData.length}, 過濾後=${filteredEvents.length}`);
  }, []);
  ...
  // 設定狀態過濾器並更新事件
  const handleSetStatusFilter = useCallback((filter: 'all' | 'upcoming' | 'ongoing' | 'completed') => {
    setStatusFilter(filter);
    updateFilteredEvents(allEvents, filter);
    
    logger.debug(`更新狀態過濾器: ${filter}`);
  }, [allEvents, updateFilteredEvents]);
