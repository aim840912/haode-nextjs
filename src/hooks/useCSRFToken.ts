...
export function useCSRFToken(): UseCSRFTokenReturn {
  ...
  const fetchToken = useCallback(
    async (forceRefresh = false): Promise<void> => {
      ...
      try {
        ...
        logger.error('Failed to fetch CSRF token', error instanceof Error ? error : undefined, {
  ...
  const clearToken = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      logger.warn('Failed to clear token on server', {
        metadata: { error: error instanceof Error ? error.message : String(error) },
      })
    }
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      logger.debug('CSRF cookie token found', {
        metadata: { tokenPreview: existingToken ? existingToken.substring(0, 8) + '...' : 'none' },
      })
    }
...
export function CSRFTokenProvider({ children }: CSRFTokenProviderProps) {
  ...
  return React.createElement(CSRFTokenContext.Provider, { value: csrfToken }, children)
}
