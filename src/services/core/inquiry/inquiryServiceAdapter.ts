...
import { InquiryServiceSimple, inquiryServiceSimple } from './inquiryServiceSimple'
import {
  InquiryService,
  InquiryWithItems,
...
export class InquiryServiceAdapter implements InquiryService {
  private readonly serviceV2: InquiryServiceSimple

  constructor(serviceV2Instance?: InquiryServiceSimple) {
    this.serviceV2 = serviceV2Instance || inquiryServiceSimple
...
export async function checkInquiryServiceHealth(): Promise<{
  ...
  try {
    // 簡單的健康檢查 - 嘗試查詢服務是否可用
    await inquiryServiceSimple.getAllInquiries({ limit: 1 })
    ...
    return {
      ...
      details: {
        adapterActive: true,
        serviceType: 'InquiryServiceSimple',
