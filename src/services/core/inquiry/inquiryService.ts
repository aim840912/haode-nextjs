...
import {
  InquiryService as IInquiryService,
  InquiryWithItems,
...
/**
 * 詢問服務實作
 * 使用 Simple 架構（直接使用 Supabase，參考 Product 模組的統一架構）
 */
export class InquiryService implements IInquiryService {
    ...
    try {
      ...
      if (inquiry.inquiry_type === 'product' && inquiry.inquiry_items.length > 0) {
        ...
        if (status === 'confirmed' && previousStatus !== 'confirmed') {
          await this.reserveInventory(inquiryId, inquiry.inquiry_items)
        ...
        if (status === 'completed' && previousStatus === 'confirmed') {
          await this.finalizeInventory(inquiry.inquiry_items)
        ...
        if (status === 'cancelled' && previousStatus === 'confirmed') {
          await this.releaseInventory(inquiry.inquiry_items)
   ...
   * 在詢問單確認時調用
   */
  private async reserveInventory(inquiryId: string, items: InquiryItem[]): Promise<void> {
    const client = this.getSupabaseClient()
   ...
   * 在詢問單取消時調用
   */
  private async releaseInventory(items: InquiryItem[]): Promise<void> {
    const client = this.getSupabaseClient()
   ...
   * 在詢問單交易完成時調用
   */
  private async finalizeInventory(items: InquiryItem[]): Promise<void> {
    const client = this.getSupabaseClient()
