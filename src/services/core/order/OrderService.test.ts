...
describe('OrderService', () => {
  ...
  describe('getOrderItems', () => {
    it('應該正確取得訂單項目', async () => {
      ...
      // Mock from('order_items').select().eq()
      mockFrom.mockImplementationOnce(() => ({
        select: () => ({
          eq: () =>
            Promise.resolve({
      ...
      const result = await service.getOrderItems('order-123')
    ...
    it('應該處理空項目', async () => {
      // Mock from('order_items').select().eq() 返回空陣列
      mockFrom.mockImplementationOnce(() => ({
        select: () => ({
          eq: () =>
            Promise.resolve({
    ...
    it('應該處理資料庫查詢錯誤', async () => {
      // Mock from('order_items').select().eq() 返回錯誤
      mockFrom.mockImplementationOnce(() => ({
        select: () => ({
          eq: () =>
            Promise.resolve({
      ...
      await expect(service.getOrderItems('order-123')).rejects.toThrow()
    })
  })
  ...
  describe('generateOrderNumber', () => {
    it('應該成功生成訂單編號', async () => {
      ...
      expect(result).toBe('ORD-20240101-0001')
    ...
    it('應該處理 RPC 錯誤', async () => {
      mockRpc.mockResolvedValueOnce({
        data: null,
        error: { code: 'RPC_ERROR', message: 'Failed' },
      })
  ...
  describe('calculateShippingFee', () => {
    ...
    it('應該返回 0 當訂單金額超過免運門檻', async () => {
      ...
      const fee = await service.calculateShippingFee(items, mockAddress)

      expect(fee).toBe(0)
    })

    it('應該返回基本運費當訂單金額未達免運門檻', async () => {
      const items = [
        {
          subtotal: 500,
        } as any,
    ...
    it('應該加收偏遠地區運費', async () => {
      const remoteAddress: ShippingAddress = {
        ...mockAddress,
        city: '離島地區',
      }
      ...
      const fee = await service.calculateShippingFee(items, remoteAddress)

      expect(fee).toBe(100) // 60 + 40
    })
  })

  describe('calculateTax', () => {
    it('應該返回 0（台灣食品類商品免營業稅）', () => {
      const tax = service.calculateTax(1000)
