 ...
 * 支援分頁查詢的服務應實作此介面
 */
export interface PaginatedService<T, CreateDTO = Partial<T>, UpdateDTO = Partial<T>>
 ...
 * 支援全文搜尋的服務應實作此介面
 */
export interface SearchableService<T, CreateDTO = Partial<T>, UpdateDTO = Partial<T>>
  extends BaseService<T, CreateDTO, UpdateDTO> {
  /**
   * 全文搜尋實體
   * @param query 搜尋關鍵字
 ...
 * 支援批次操作的服務應實作此介面
 */
export interface BatchService<T, CreateDTO = Partial<T>, UpdateDTO = Partial<T>>
  extends BaseService<T, CreateDTO, UpdateDTO> {
  /**
   * 批次建立實體
