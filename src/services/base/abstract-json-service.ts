 ...
 * 支援記憶體快取、備份和搜尋功能
 */
export abstract class AbstractJsonService<
    T extends JsonEntity,
    CreateDTO = Partial<T>,
    UpdateDTO = Partial<T>,
  >
  implements
    BaseService<T, CreateDTO, UpdateDTO>,
    PaginatedService<T, CreateDTO, UpdateDTO>,
    SearchableService<T, CreateDTO, UpdateDTO>
{
  ...
  constructor(config: JsonServiceConfig, searchConfig?: SearchConfig) {
    this.config = {
      enableCache: true,
      cacheTTL: 300, // 5分鐘
  ...
  private async cleanupOldBackups(): Promise<void> {
    try {
      ...
      await Promise.all(
        filesToDelete.map(({ file }) => fs.unlink(path.join(dir, file)).catch(() => {}))
      )
    } catch (error) {
      // 清理失敗不影響主要操作
      logger.warn('清理舊備份失敗', {
