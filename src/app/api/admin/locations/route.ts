...
async function handleDELETE(request: NextRequest) {
  ...
  if (fetchError && fetchError.code !== 'PGRST116') {
    apiLogger.error(`Error fetching location ${id} for audit:`, fetchError)
  }
  ...
  try {
    apiLogger.info(`🗑️ 開始為地點 ${id} 清理圖片...`)
    deletedImageCount = await unifiedImageService.deleteEntityImages('locations', id)
    imageCleanupSuccess = true
    apiLogger.info(`✅ 地點 ${id} 的圖片清理完成 - 刪除了 ${deletedImageCount} 個檔案`)
  } catch (storageError) {
    imageCleanupError = (storageError as Error).message
    apiLogger.warn(`⚠️ 地點 ${id} 圖片清理過程發生異常`, {
  ...
  try {
    ...
    await auditService.log({
      ...
      user_role: 'admin',
      action: 'delete',
      resource_type: 'location' as const,
      resource_id: id,
      resource_details: locationData as unknown as Record<string, unknown>,
      metadata: {
        imageCleanup: {
    ...
    apiLogger.warn('Failed to log location deletion audit', {
      metadata: { error: (auditError as Error).message },
    })
  }

  apiLogger.info('🔄 地點刪除完成，快取已自動更新')
