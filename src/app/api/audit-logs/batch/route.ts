...
async function handleDeleteByIds(
  ...
  await auditLogService
    .log({
      ...
      user_role: (profile.role || 'customer') as 'customer' | 'admin',
      action: 'delete',
  ...
  return success({ deleted_count: deletedCount }, `成功刪除 ${deletedCount} 筆審計日誌`)
}
...
async function handleDeleteByFilters(
  ...
  return success({ deleted_count: deletedCount }, `成功刪除 ${deletedCount} 筆符合條件的審計日誌`)
}
...
async function handleCleanupOld(
  ...
  try {
    // 呼叫資料庫清理函數
    // 注意：SQL 函數定義與 TypeScript 類型定義不一致，使用類型斷言
