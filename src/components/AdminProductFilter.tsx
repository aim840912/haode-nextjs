...
export interface AdminFilterState {
  ...
  sortBy: 'name' | 'price_low' | 'price_high' | 'category' | 'inventory' | 'created_desc' | 'created_asc'
}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFilters(prev => ({ ...prev, sortBy: e.target.value as AdminFilterState['sortBy'] }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
              >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {availableCategories.map(category => (
                <label key={category} className="flex items-center bg-gray-50 px-3 py-2 rounded-lg hover:bg-gray-100 cursor-pointer">
                  <input
                    type="checkbox"
                ...
                {[
                  ...
                  <label key={option.value} className="flex items-center">
                    <input
                      ...
                      onChange={(e) => setFilters(prev => ({ ...prev, availability: e.target.value as AdminFilterState['availability'] }))}
                ...
                {[
                  ...
                  <label key={option.value} className="flex items-center">
                    <input
                      ...
                      onChange={(e) => setFilters(prev => ({ ...prev, status: e.target.value as AdminFilterState['status'] }))}
                      className="text-amber-600 focus:ring-amber-500 mr-2"
                    />
                    <span className="text-sm text-gray-700">{option.label}</span>
                  </label>
                ...
                {[
                  ...
                  <label key={option.value} className="flex items-center">
                    <input
                      ...
                      onChange={(e) => setFilters(prev => ({ ...prev, catalogVisibility: e.target.value as AdminFilterState['catalogVisibility'] }))}
                      className="text-amber-600 focus:ring-amber-500 mr-2"
                    />
                    <span className="text-sm text-gray-700">{option.label}</span>
                  </label>
